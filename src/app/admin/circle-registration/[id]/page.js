...
export default function RegistrationReviewPage({ params }) {
  ...
  const handleDeleteRegistration = async () => {
    ...
    try {
      ...
      router.push('/admin/circle-registration');
    } catch (err) {
      setError(err.message);
      setActionLoading(false);
    }
          ...
          {confirmingDelete ? (
            <div className="flex gap-4">
              <button
                ...
                style={{ padding: '0.4rem 0.8rem', fontSize: '0.85rem', background: 'var(--danger)', border: 'none' }}
              >
            ...
            <button
              ...
              style={{ width: '36px', height: '36px', padding: 0, color: 'var(--danger)', borderColor: 'var(--danger)', borderRadius: '50%', flexShrink: 0 }}
              aria-label={`Delete registration for ${registration.circleNameEn}`}

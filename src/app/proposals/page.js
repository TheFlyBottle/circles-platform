    ...
    return (
        <div className="animate-fade-in" style={{ paddingBottom: '4rem' }}>
            ...
            <header className="text-center mb-8">
                <h2 className="font-serif" style={{ fontSize: '2.5rem', marginBottom: '0.5rem', color: 'var(--accent-primary)' }}>Submit a Proposal</h2>
                <h3 className="font-sans dir-rtl" style={{ fontSize: '1.5rem', fontWeight: 300, color: 'var(--text-secondary)' }}>ثبت پیشنهاد حلقه</h3>
                <p style={{ color: 'var(--text-secondary)', marginTop: '1rem', maxWidth: '600px', margin: '1rem auto' }}>
                    Interested in starting a new reading group or study circle? Fill out the form below to propose your idea to The Fly Bottle team.
                </p>
            </header>
                    ...
                    <div className="form-group mb-8" style={{ background: 'var(--background)', padding: '1.5rem', borderRadius: 'var(--border-radius)', border: '1px solid var(--border-color)' }}>
                        <div className="checkbox-group" style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start', justifyContent: 'flex-end', flexDirection: 'row-reverse' }}>
                            ...
                            <label htmlFor="termsCheckbox" className="dir-rtl text-right" style={{ fontSize: '0.95rem', flex: 1, cursor: 'pointer' }}>
                                من <a href="https://www.theflybottle.org/circles/about-circles/code-of-conduct" target="_blank" rel="noopener noreferrer" style={{ fontWeight: 600, textDecoration: 'underline', color: 'var(--accent-primary)' }}>شرایط استفاده از خدمات حلقه‌های مگس در بطری</a> را خوانده‌ام و با آن موافقم. <span style={{color: 'var(--danger)'}}>*</span>
                            </label>
                        </div>
                    </div>

                    <button type="submit" className="btn-primary w-full" style={{ padding: '1.25rem', fontSize: '1.1rem' }} disabled={loading}>
                        {loading ? 'Submitting / در حال ثبت...' : 'Submit Proposal / ثبت پیشنهاد'}
                    </button>
                </form>
            </div>
        </div>
